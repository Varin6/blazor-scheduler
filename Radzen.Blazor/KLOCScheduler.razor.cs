...
namespace Radzen.Blazor
{
    ...
    /// </code>
    /// </example>
    public partial class KLOCScheduler<TItem> : RadzenComponent, IKLOCScheduler
    {
        /// <summary>
        ...
        /// <summary>
        /// Specifies the property of <typeparamref name="TItem" /> which will set <see cref="AppointmentData.Name" />.
        /// </summary>
        /// <value>The name of the property. Must be a <c>DateTime</c> property.</value>
        [Parameter]
        ...
        IEnumerable<KLOCSchedulerData> schedulerData;
        DateTime rangeStart;
        DateTime rangeEnd;
        Func<TItem, string> nameGetter;
        Func<TItem, AppointmentData> appointmentDataGetter;
        ...
        public IEnumerable<KLOCSchedulerData> GetAllAccountData(DateTime start, DateTime end)
        {
            
            
            if (Data == null)
            {
                return Array.Empty<KLOCSchedulerData>();
            }

            if (start == rangeStart && end == rangeEnd && schedulerData != null)
            {
            ...
            schedulerData = Data.AsQueryable()
                               // .Where(predicate, start, end)
                               .ToList()
                               .Select(item => new KLOCSchedulerData {Start = startGetter(item), Name = nameGetter(item),  Data = item });
                               // .Select(item => new KLOCSchedulerData { Name = nameGetter(item), AppointmentData = appointmentDataGetter(item), Data = item });
